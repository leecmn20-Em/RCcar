...
현재 Wheel RPM 추정기는 encoder pulse가 100ms 이상 들어오지 않으면 마지막 순간 RPM을 그대로 고정하지 않고, 무펄스 시간으로 계산한 상한까지 점진적으로 낮춥니다. 마지막 pulse 주기 2배와 300ms 중 큰 시간이 지나면 RPM을 0으로 확정하며 timeout 상한은 1초입니다. 따라서 일반적인 90 RPM 주행에서는 마지막 pulse 이후 약 0.7~0.9초에 `STOP`이 처음 전송될 수 있고, 정지 직전 pulse 주기가 매우 길었다면 최대 약 1.4~1.6초까지 걸릴 수 있습니다. 실제 지연은 200ms 전송 주기와 encoder 특성에 따라 달라집니다.
...
`ino/build/` 아래의 생성 파일은 다른 개발 환경에서 만들어진 과거 산출물일 수 있으므로 flash 입력으로 사용하지 않습니다. Uno에는 반드시 현재 `ino/ino.ino` 소스를 새로 compile한 firmware를 업로드합니다.

1. ESP32 DevKit V1의 Servo signal 배선이 GPIO 18/19/21/22 순서와 일치하는지 확인
